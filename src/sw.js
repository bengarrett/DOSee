/*globals articleHandler workbox*/
...
if (workbox) {
  ...
  workbox.routing.setDefaultHandler(async (args) => {
    let response = await caches.match(args.request);
    if (response) {
      return response;
    }
    // If not in cache, try network
    try {
      response = await fetch(args.request);
      return response;
    } catch {
  ...
  self.addEventListener('activate', (event) => {
    ...
    // Claim clients immediately from the service worker
